...
double rectP(double x, double  y) {
	return (x * y);
}
double rectP(double x, double y) {
	return x * x, y* y;
...
int main() {
	...
	//Proc3.Описать 2 функции aMean(X, Y), gMean(X, Y), 
	...
	/*double a;
	for (int i = 1; i <= 5; i++) {
		cout << "Введите сторону треуголника ";
		cin >> a;
	...
	//по координатам(x1, y1), (x2, y2) его противоположных вершин.С помощью этих функций найти периметры
